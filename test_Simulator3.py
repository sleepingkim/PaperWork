...
try:
    ...
    print(f"[{t_now}] Please ensure Ursina, Scikit-learn, NumPy, Pandas, Matplotlib, and Open3D are installed.")
    print(f"[{t_now}] Example install command: pip install ursina scikit-learn numpy pandas matplotlib open3d")
    sys.exit(1) 
...
IOU_THRESHOLD = 0.5
AREA_OVERLAP_RATIO_THRESHOLD = 0.3
...
def evaluate_and_plot_algorithms(history, current_gt_obstacle_df, gnd_info, show_plots=True): 
    ...
    try: 
            ...
            for algo_config in algorithms_to_run:
                ...
                if len(points_for_algo_input) >= min_samples_for_algo: 
                    try:
                        ...
                        if algorithm_name == "KMEANS": current_algo_model_params["n_clusters"] = min_samples_for_algo
                        elif algorithm_name == "GMM": current_algo_model_params["n_components"] = min_samples_for_algo
                        ...
                        if labels.size > 0 and len(non_ground_points_orig) == len(labels): 
                            ...
                            if show_plots: colors_map = plt.cm.get_cmap('Spectral', len(unique_labels) if len(unique_labels) > 0 else 1) 
                            num_clusters = len(unique_labels) - (1 if -1 in labels and algorithm_name == "DBSCAN" else 0) 
                            if algorithm_name != "DBSCAN": num_clusters = len(unique_labels) 
                            cluster_label_printed = False
                            for k_idx, k_label in enumerate(unique_labels):
                                if algorithm_name == "DBSCAN" and k_label == -1: continue 
                        ...
                        if show_plots and ax3: ax3.text(0,0,0,f'{algorithm_name} Error',c='red')
                else: 
                    print(f"  Not enough points for {algorithm_name} (or k=0).")
                    if show_plots and ax3: ax3.text(0,0,0,f'No Points/k=0 for {algorithm_name}',c='gray')
                ...
                if show_plots and ax3:
                    ...
                    if algorithm_name == "DBSCAN": ax3_title_text += f"eps={model_params['eps']}, min_s={model_params['min_samples']}, Y-Scl={y_scale_factor_algo}"
                ...
                if show_plots: plt.tight_layout(rect=[0, 0.05, 1, 0.92]) 
                ...
                if show_plots: fig_algo.text(0.5, 0.02, eval_text, ha='center', va='bottom', fontsize=9, bbox=dict(boxstyle='round,pad=0.3', fc='wheat', alpha=0.7))
                
                all_algorithms_results_summary_for_all_eval_methods.append({
                    "Algorithm": algorithm_name, 
                    "Params": str(model_params) + (f", YScl:{y_scale_factor_algo}" if y_scale_factor_algo!=1.0 else ""),
        ...
        if show_plots: 
            print(f"DEBUG: Number of open Matplotlib figures before show(): {len(plt.get_fignums())}")
            if len(plt.get_fignums()) == 0:
                print("DEBUG: No figures were created or they were closed before plt.show().")
            else:
                print(f"DEBUG: Figure numbers are: {plt.get_fignums()}")
            plt.show(block=True) 
...
def input(key):
    ...
    if key in ('0', '4', '5', '6', '7', '8', '9'): 
    ...
    elif key=='q up': rotation_states['left']=False # 이 부분이 제대로 되어 있는지 확인
    elif key in ['e','e hold']: rotation_states['right']=True
    elif key=='e up': rotation_states['right']=False 
    elif key in ['m','M']: 
        if scan_history:
            print("DEBUG: 'M' key pressed. Calling evaluate_and_plot_algorithms with show_plots=True")
        ...
        if not simulation_running and scan_history:
            print("Manually evaluating and saving results...")
            summary_df_results = evaluate_and_plot_algorithms(scan_history, obstacle_df, ground_info, show_plots=False) 
            if summary_df_results is not None and not summary_df_results.empty:
                save_performance_summary_to_csv(summary_df_results)
            else:
...
def update():
    ...
    if simulation_running:
        ...
        if simulation_time_elapsed >= TOTAL_SIMULATION_DURATION and simulation_running: 
            ...
            visualize_path([]) 
            if scan_history:
                print("Automatically saving results...")
                summary_df_results = evaluate_and_plot_algorithms(scan_history, obstacle_df, ground_info, show_plots=False) 
                if summary_df_results is not None and not summary_df_results.empty:
                    save_performance_summary_to_csv(summary_df_results)
                else:
